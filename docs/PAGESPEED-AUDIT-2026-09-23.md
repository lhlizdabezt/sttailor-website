...
- The 77 Best Practices result is driven by third-party cookie warnings associated with the requested Microsoft Clarity instrumentation. Removing Clarity solely to improve that number would remove recordings and heatmaps; the site keeps it.

After deployment, rerun the same mobile and desktop suite against production and compare medians, audit details, and Core Web Vitals field data separately. Lighthouse performance can vary between runs because of network and CPU conditions.
